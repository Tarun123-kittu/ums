...
const authenticateToken = (req, res, next) => {
    ...
    if (!authHeader) {
        return res.status(401).json({ type: "error", message: "Authorization header missing" });
    }
    // checking is token bearer or not
    const token = authHeader.split(' ')[1];

    if (!token) {
        return res.status(401).json({ type: "error", message: "Token missing" });
    }
